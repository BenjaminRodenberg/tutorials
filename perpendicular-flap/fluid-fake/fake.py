...
F_max = 1.0 # F_max >= 5 will cause problems with linear structure model
...
force = lambda t: sigmoid(t) * F_max * vertices[:, 1] / H  # linearly increasing load in y direction

if interface.requires_initial_data():
    write_data = np.zeros((n, dimensions))
    write_data[:, 0] = force(0)
    write_data[:, 1] = 0
